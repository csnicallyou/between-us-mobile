...
`POST /chat/messages` sends a generic push ("Новое сообщение в общем чате", no message content) to the other pair member via `server/src/lib/push.ts` (Expo push service), skipping tokens inside their configured quiet hours. Creating a plan, journal entry, memory, or agreement sends the same content-free category notification. Agreement and anniversary reminders also avoid entry titles and other sensitive text on the lock screen. Failures never affect the underlying action. Real delivery on this beta's free-signed sideload IPA is unverified (push notification entitlements generally need a paid Apple Developer account).
