...
export async function sendPushToUser(db: Pool, userId: string, notification: PushNotificationInput): Promise<void> {
  const result = await db.query<{ expo_push_token: string; quiet_hours_start: number | null; quiet_hours_end: number | null; timezone: string | null }>(
    "SELECT expo_push_token,quiet_hours_start,quiet_hours_end,timezone FROM push_tokens WHERE user_id=$1",
    [userId]
  );
