...
export default function AgreementsScreen() {
  ...
  return <Screen header={<SubpageHeader title="Договорённости" subtitle="Правила, которые вы формулируете сами и подтверждаете каждый от своего имени." />}><AppButton label="Новая договорённость" onPress={() => begin()} /><View style={styles.list}>{snapshot.agreements.length ? snapshot.agreements.map((item) => <Pressable key={item.id} onLongPress={() => remove(item)} onPress={() => begin(item)}><Surface><Text style={styles.title}>{item.title}</Text>{item.description ? <Text style={styles.copy}>{item.description}</Text> : null}<View style={styles.acceptance}><Text style={[styles.person, item.acceptedBy.anton && styles.accepted]}>Антон {item.acceptedBy.anton ? "согласен" : "не подтвердил"}</Text><Text style={[styles.person, item.acceptedBy.lisa && styles.accepted]}>Лиза {item.acceptedBy.lisa ? "согласна" : "не подтвердила"}</Text></View><AppButton label={item.acceptedBy[snapshot.currentMemberId] ? "Отозвать моё согласие" : "Я согласен(на)"} onPress={() => toggleAgreement(item.id)} variant="secondary" /></Surface></Pressable>) : <Surface><Text style={styles.empty}>Добавьте первую общую договорённость.</Text></Surface>}</View>
