...
const conflictTopics = { availability: "Доступность", trust: "Доверие", boundaries: "Границы", communication: "Общение", other: "Другое" } as const;
  ...
  return <V2Screen>
    ...
    <View style={s.bento}>
      ...
      <View style={s.pair}>
        ...
        <OrbSinkItem style={s.half}><Pressable onPress={() => go("/agreements")}><V2Glass radius={24} style={s.tile}><View style={s.tileHead}><Text style={s.tileNumber}>{snapshot.agreements.length}</Text><Text style={s.tileUnit}>{plural(snapshot.agreements.length, ["правило", "правила", "правил"])}</Text></View><Text style={s.tileName}>Договорённости</Text><Text style={s.tileSub}>{accepted} {plural(accepted, ["подтверждена", "подтверждены", "подтверждено"])} обоими</Text><View style={s.mini}>{waiting > 0 && <View style={s.coralDot} />}<Text style={s.miniText}>{waiting ? `${waiting} ${plural(waiting, ["ждёт", "ждут", "ждут"])} подтверждения` : "Всё подтверждено"}</Text></View></V2Glass></Pressable></OrbSinkItem>
      </View>
      <OrbSinkItem><Pressable onPress={() => go("/conflicts")}><V2Glass radius={26} style={s.review}>
        <View style={s.reviewRow}><View style={s.reviewNumber}><Text style={s.reviewNumberText}>{snapshot.conflicts.length}</Text><Text numberOfLines={1} style={s.reviewUnit}>эпизодов</Text></View><View style={s.flex}><Text style={s.sectionTitle}>Разбор ссор</Text><Text style={s.sectionMeta}>Не рейтинг виноватых, а архив выводов</Text></View><Ionicons color={c.faint} name="chevron-forward" size={18} /></View>
        {latestConflict && <View style={s.latest}><Text style={s.latestKicker}>Последний разбор · {conflictTopics[latestConflict.topic]} · {formatDateSafe(latestConflict.date, { day: "numeric", month: "long" })}</Text><Text numberOfLines={2} style={s.latestTitle}>{latestConflict.lesson || latestConflict.title}</Text></View>}
      </V2Glass></Pressable></OrbSinkItem>
    </View>
  </V2Screen>;
}
...
const s = StyleSheet.create({
  ...
  review: { paddingHorizontal: 18, paddingVertical: 17 }, reviewRow: { alignItems: "center", flexDirection: "row", gap: 13 }, reviewNumber: { alignItems: "center", backgroundColor: "rgba(255,255,255,.16)", borderRadius: 17, height: 52, justifyContent: "center", width: 62 }, reviewNumberText: { color: c.text, fontFamily: "GolosText", fontSize: 19, fontWeight: "600", lineHeight: 21 }, reviewUnit: { color: c.faint, fontFamily: "GolosText", fontSize: 7.5, letterSpacing: .35, textTransform: "uppercase" }, latest: { backgroundColor: "rgba(255,255,255,.13)", borderRadius: 15, marginTop: 13, paddingHorizontal: 13, paddingVertical: 11 }, latestKicker: { color: c.faint, fontFamily: "GolosText", fontSize: 9.5, fontWeight: "600", letterSpacing: .9, textTransform: "uppercase" }, latestTitle: { color: c.text, fontFamily: "GolosText", fontSize: 13.5, fontWeight: "500", lineHeight: 18, marginTop: 5 },
});
